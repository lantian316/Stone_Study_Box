...
def main():    
    ...
    if createdir_bool:           #重新调用compareme函数，重新变异新创建目录的内容
        ...
        for item in source_files:              #获取源目录差异路径清单，对应替换成备份目录
            ...
            #就是上面这一步的路径替换没有替换成功，导致后面无法复制，这里不知道怎么搞了。
            destination_files.append(destination_dir)
            
    print("update item:")
    print("source_files    ",source_files)           #输出更新列表清单
    print("destination_files    ",destination_files)
    destination_files = ['F:\\python_study_box\\Module\\filecmp\\dir4\\8767.txt',
                         'F:\\python_study_box\\Module\\filecmp\\dir4\\test',
                         'F:\\python_study_box\\Module\\filecmp\\dir4\\987.txt',
                         'F:\\python_study_box\\Module\\filecmp\\dir4\\jhk.txt']
    print("destination_files    ", destination_files)
